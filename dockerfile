...
# CMD: Dump environment variables at runtime, set up cron, and start it
CMD printenv > /etc/environment && \
    echo "$CRON_SCHEDULE /usr/local/bin/python /usr/src/app/update_ip.py >> /var/log/cron.log 2>&1" > /etc/cron.d/update_ip_cron && \
